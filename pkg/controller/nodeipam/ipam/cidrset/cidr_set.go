...
func (s *CidrSet) GetCapacity() int {
	capacity := 0
...
func (s *CidrSet) AllocateNext() (*net.IPNet, error) {
	s.Lock()
	defer s.Unlock()
	return s.allocateNextImpl()
}

func (s *CidrSet) allocateNextImpl() (*net.IPNet, error) {
	nextUnused := -1
	for i := 0; i < s.MaxCIDRs; i++ {
	...
	klog.Infof("current available range in cidrset %v: %v, candidate: %v", s.ClusterCIDR.String(), s.used.Text(2), nextUnused)
	if nextUnused == -1 {
...
func (s *CidrSet) Release(cidr *net.IPNet) error {
	s.Lock()
	defer s.Unlock()
	return s.releaseImpl(cidr)
}

func (s *CidrSet) releaseImpl(cidr *net.IPNet) error {
	...
	klog.Infof("current available range in cidrset %v: %v, next candidate: %v", s.ClusterCIDR.String(), s.used.Text(2), s.nextCandidate)
	return nil
}
...
func (s *CidrSet) AllocateNextN(n int) ([]*net.IPNet, error) {
	// lock to avoid fragmentation
	s.Lock()
	defer s.Unlock()
	...
	if err != nil {
		for _, cidr := range allocatedCidrs {
			if err := s.releaseImpl(cidr); err != nil {
				klog.Errorf("failed to release cidr: %v", cidr.String())
			}
		}
		return nil, err
	}

	v4, v6 := ip.CoalesceCIDRs(allocatedCidrs)
	if s.ClusterIP.To4() != nil {
		return v4, nil
	}
	return v6, nil
}
