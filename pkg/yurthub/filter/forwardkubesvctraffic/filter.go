...
func Register(filters *filter.Filters) {
	filters.Register(filter.ForwardKubeSvcTrafficFilterName, func() (filter.ObjectFilter, error) {
...
func (fkst *forwardKubeSVCTrafficFilter) Name() string {
	return filter.ForwardKubeSvcTrafficFilterName
}
...
func (fkst *forwardKubeSVCTrafficFilter) Filter(obj runtime.Object, stopCh <-chan struct{}) runtime.Object {
	...
	case *discovery.EndpointSliceList:
		for i := range v.Items {
			if mutated := fkst.mutateDefaultKubernetesEps(&v.Items[i]); mutated {
				break
			}
		}
		return v
...
func (fkst *forwardKubeSVCTrafficFilter) mutateDefaultKubernetesEps(eps *discovery.EndpointSlice) bool {
	mutated := false
	trueCondition := true
	if eps.Namespace == KubeSVCNamespace && eps.Name == KubeSVCName {
		...
		klog.V(2).Infof("mutate default/kubernetes endpointslice to %v", *eps)
	}
	return mutated
}
