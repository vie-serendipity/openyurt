...
func TestName(t *testing.T) {
	...
	if fkst.Name() != filter.ForwardKubeSvcTrafficFilterName {
		t.Errorf("expect %s, but got %s", filter.ForwardKubeSvcTrafficFilterName, fkst.Name())
	}
}
...
func TestFilter(t *testing.T) {
	...
	testcases := map[string]struct {
		...
		"endpointsliceList contains kubernetes endpointslice": {
			host: "169.251.2.1",
			port: fmt.Sprintf("%d", svcPort),
		...
		"endpointsliceList doesn't contain kubernetes endpointslice": {
			host: "169.251.2.1",
			port: fmt.Sprintf("%d", svcPort),
			responseObject: &discovery.EndpointSliceList{
				Items: []discovery.EndpointSlice{
