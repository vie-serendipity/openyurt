...
func (applier *ModelApplier) Apply(reqCtx *RequestContext, local *elbmodel.EdgeLoadBalancer) (*elbmodel.EdgeLoadBalancer, error) {

	remote, err := applier.ELBMgr.BuildRemoteModel(reqCtx)
	if err != nil {
	...
	if err = applier.applyEipAttribute(reqCtx, local, remote); err != nil {
...
func (applier *ModelApplier) applyLoadBalancerAttribute(reqCtx *RequestContext, localModel, remoteModel *elbmodel.EdgeLoadBalancer) error {
	...
	if needDeleteLoadBalancer(reqCtx.PoolService) {
		if remoteModel.GetLoadBalancerId() == "" {
			klog.Info(Format("elb does not exist for poolservice %s", Key(reqCtx.PoolService)))
			return nil
		}
		if !remoteModel.IsUserManaged() {
			err := applier.ELBMgr.DeleteELB(reqCtx, remoteModel)
			if err != nil {
				return err
			}
			klog.InfoS(fmt.Sprintf("successfully delete elb %s", remoteModel.GetLoadBalancerId()), "poolservice", Key(reqCtx.PoolService))
			return nil
		}
		klog.InfoS(fmt.Sprintf("elb %s is manageed by user, skip delete it", remoteModel.GetLoadBalancerId()), "poolservice", Key(reqCtx.PoolService))
		return nil
	}

	if remoteModel.GetLoadBalancerId() != "" {
		klog.Infof(Format("elb %s has been created for poolservice %s", remoteModel.GetLoadBalancerId(), Key(reqCtx.PoolService)))
		return nil
	}
	...
	klog.InfoS(fmt.Sprintf("successfully create elb %s", remoteModel.GetLoadBalancerId()), "poolservice", Key(reqCtx.PoolService))
	return nil
}

func (applier *ModelApplier) applyEipAttribute(reqCtx *RequestContext, localModel, remoteModel *elbmodel.EdgeLoadBalancer) error {
	if localModel.NamespacedName.String() != remoteModel.NamespacedName.String() {
		return fmt.Errorf("models for different svc, local [%s], remote [%s]",
			localModel.NamespacedName.String(), remoteModel.NamespacedName.String())
	}

	// delete eip that is managed by ecm when pool service is deleted and address type from internet to intranet
	if needDeleteLoadBalancer(reqCtx.PoolService) || localModel.LoadBalancerAttribute.AddressType == elbmodel.IntranetAddressType {
		if remoteModel.GetEIPId() == "" {
			klog.Info(Format("eip does not exist for service %s", Key(reqCtx.PoolService)))
			return nil
		}
		if !remoteModel.IsUserManaged() {
			...
			klog.InfoS(fmt.Sprintf("successfully delete eip %s", remoteModel.GetEIPId()), "poolservice", Key(reqCtx.PoolService))
			return nil
		}
		klog.InfoS(fmt.Sprintf("eip %s is manageed by user, skip delete it", remoteModel.GetEIPId()), "poolservice", Key(reqCtx.PoolService))
		return nil
	}

	if localModel.IsUserManaged() {
	...
	if remoteModel.GetEIPId() == "" {
		...
		klog.InfoS(fmt.Sprintf("successfully create eip %s", remoteModel.GetEIPId()), "poolservice", Key(reqCtx.PoolService))
	}
	if remoteModel.EipAttribute.InstanceId == "" {
		...
		klog.InfoS(fmt.Sprintf("successfully associate eip %s to elb %s", remoteModel.GetEIPId(), remoteModel.GetLoadBalancerId()), "poolservice", Key(reqCtx.PoolService))
	}
	if localModel.EipAttribute.Bandwidth != remoteModel.EipAttribute.Bandwidth {
		...
		klog.InfoS(fmt.Sprintf("successfully update eip %s bandwidth into %d ", remoteModel.GetEIPId(), localModel.EipAttribute.Bandwidth), "poolservice", Key(reqCtx.PoolService))
	}
	return nil
}
